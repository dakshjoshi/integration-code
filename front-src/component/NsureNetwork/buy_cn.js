...
/*  const someDATA = JSON.stringify({
  ...
  const someDATA = JSON.stringify(getDiggerDATA({
    account: address,
    currencyType: currency,
    amount: _amount,
    cost: _cost,
  ...
  const buyIt = async () => {
    ...
    bodyParameters.params = someDATA;
    bodyParameters.sign = {
      result: signature,
    };
 ...
 * @param product
 * @param nonce
 * @returns {{types: {sign: [{name: string, type: string}, {name: string, type: string}, {name: string, type: string}, {name: string, type: string}, {name: string, type: string}, null, null], EIP712Domain: [{name: string, type: string}, {name: string, type: string}]}, primaryType: string, domain: {chainId: number, name: string}, message: {currencyType, amount, period, product, deadline: number, nonce, account}}}
 */
function getDiggerDATA(message){
  return {
    domain: {
      name: "sign",
      chainId: 42,
